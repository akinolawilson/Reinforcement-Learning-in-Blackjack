...
#        self.hit = False
    ...
    def newRound(self): #dont need that bool 
    ...
    def score(self):
        ...
        if self.hitRound == False:
            ...
            try:
                self.dealerPartialScore += int(self.dealerHand[0])    
            except ValueError:
                pass
            if self.dealerHand[0] in ('J','K','Q','A'):
                if str(self.dealerHand[0]) == 'A':
                    self.dealerPartialScore += 11
                else:
                    self.dealerPartialScore += 10
        #The hit round 
        ##########################################################################
        if self.hitRound == True:
            ...
            if self.agentHand[-1] in ('J','K','Q','A'):
                if self.agentHand[-1] == 'A':                                   
                    self.aceCount += 1
                    while self.hitScore + 11 > Dealing.blackJack:
    ...
    def actionOutcome(self, action): # function describes consequences of action
        ...
        if action == 0: # to stick with current cards
            if self.dealerScore < self.hitScore <= Dealing.blackJack:
                ...
                reward += 1 * int(self.hitScore**2) # increasing reward with score
